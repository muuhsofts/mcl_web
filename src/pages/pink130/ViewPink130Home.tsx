...
// Define the shape of the API response based on the Laravel controller
interface ApiResponse {
  ft_pink_130_homes?: FtPink130HomeData[];
  error?: string;
}
...
const ActionButtons: React.FC<ActionButtonsProps> = ({ ft_pink_id, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/mcl-pink-130-home/${ft_pink_id}`);
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link
        to={`/edit-mcl-pink-130-home/${ft_pink_id}`}
...
const ViewFtPink130Home: React.FC = () => {
  const [data, setData] = useState<FtPink130HomeData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedImage, setSelectedImage] = useState<string | null>(null);
  const [globalFilter, setGlobalFilter] = useState('');

  const fetchFtPink130Home = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await axiosInstance.get<ApiResponse>('/api/mcl-pink-130-home');
      const responseData = response.data.ft_pink_130_homes;
      ...
      const errorMessage =
        err.response?.data?.error || 'Failed to fetch entries.';
      setError(errorMessage);
      setData([]); // Ensure data remains an array
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('Ft Pink 130 Home Entries', 20, 10);
    autoTable(doc, {
    ...
    doc.save('ft_pink_130_home_entries.pdf');
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'FtPink130Home');
    XLSX.writeFile(workbook, 'ft_pink_130_home_entries.xlsx');
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">
            Pink 130 Home Management
          </h2>
          <Link
            to="/add/mcl-pink-130-home"
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <input
            ...
            className="px-4 py-2 w-full sm:w-64 border rounded-lg bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        <div className="overflow-x-auto">
          <table
            {...getTableProps()}
            className="w-full divide-y divide-gray-200 rounded-lg border border-gray-200"
          >
                ...
                <tr {...headerGroup.getHeaderGroupProps()}>
                  {headerGroup.headers.map((column) => (
                    <th
                      ...
                      className="px-2 sm:px-4 py-3 text-left text-sm font-semibold tracking-wider text-gray-700 uppercase"
                    >
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center mt-6 gap-4">
            <div className="flex gap-2">
              <button
                ...
                className="px-4 py-2 bg-blue-500 text-white rounded-lg disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed hover:bg-blue-600 transition"
              >
                Previous
              </button>
              <button
...
export default ViewFtPink130Home;