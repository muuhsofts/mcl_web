...
const ActionButtons: React.FC<ActionButtonsProps> = ({ benefit_home_id, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/benefities-home/${benefit_home_id}`);
      toast.success('Benefities home entry deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete benefities home entry.', { position: 'top-right' });
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit/benefityhome/${benefit_home_id}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this benefities home entry?</p>
...
export default function BenefitiesHome() {
  const [data, setData] = useState<BenefitiesHomeData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedImage, setSelectedImage] = useState<string | null>(null);

  const fetchBenefitiesHomes = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await axiosInstance.get<BenefitiesHomeData[]>('/api/benefities-home');
      setData(response.data);
    } catch (err: any) {
      const errorMessage = 'Failed to fetch benefities home entries: ' + (err.response?.data?.message || err.message || 'Unknown error');
      setError(errorMessage);
      toast.error('Failed to fetch benefities home entries.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    fetchBenefitiesHomes();
  }, [fetchBenefitiesHomes]);

  // FIX: Explicitly type the columns array
  const columns: readonly Column<BenefitiesHomeData>[] = useMemo(
    () => [
      {
        Header: '#',
        id: 'rowIndex',
        Cell: ({ row, flatRows }: CellProps<BenefitiesHomeData>) => {
      ...
      { Header: 'Heading', accessor: 'heading' },
      {
        ...
        Cell: ({ value }: CellProps<BenefitiesHomeData, string | null>) => <DescriptionCell value={value} />,
      },
      {
        Header: 'Image',
        accessor: 'home_img',
        Cell: ({ value }: CellProps<BenefitiesHomeData, string | null>) => {
      ...
      },
      {
        Header: 'Created At',
        accessor: 'created_at',
        Cell: ({ value }: CellProps<BenefitiesHomeData, string>) => new Date(value).toLocaleDateString(),
      },
      {
        Header: 'Actions',
        accessor: 'benefit_home_id',
        Cell: ({ row }: CellProps<BenefitiesHomeData>) => (
          <ActionButtons benefit_home_id={row.original.benefit_home_id} onDeletionSuccess={fetchBenefitiesHomes} />
        ),
      },
    ],
    [fetchBenefitiesHomes]
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('Benefities Home Entries', 20, 10);
    autoTable(doc, {
    ...
    doc.save('benefities_home_entries.pdf');
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'BenefitiesHomes');
    XLSX.writeFile(workbook, 'benefities_home_entries.xlsx');
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Benefities Home Management</h2>
          <Link to="/add/benefityhome" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
            <svg className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
            Create Benefities Home
          </Link>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <input
            ...
            placeholder="Search benefities home entries..."
              ...
              {page.length > 0 ? (
                ...
                <tr>
                  <td colSpan={columns.length} className="text-center py-10 text-gray-500">
                    No benefities home entries found matching your criteria.
                  </td>
                </tr>
              )}
            </tbody>
