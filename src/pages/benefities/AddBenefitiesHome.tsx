...
// FIX: Create a dedicated interface for error messages.
...
const AddBenefitiesHome = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/benefities-home', payload, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      toast.success(response.data.message || 'Benefities home entry created successfully', {
        position: 'top-right',
      });
      setTimeout(() => navigate('/benefities/home'), 2000);
    } catch (error: any) {
      const errorMessage = error.response?.data?.error || 'Failed to create benefities home entry';
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 lg:p-8 w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            />
            {/* FIX: This is now type-safe as errors.home_img is a string. */}
            {errors.home_img && (
...
export default AddBenefitiesHome;