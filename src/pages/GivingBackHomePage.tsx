...
const GivingBackHomeSlideshow: React.FC<{ setLoaded: (isLoaded: boolean) => void }> = ({ setLoaded }) => {
  ...
  if (error || data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh] text-center p-6 bg-gray-800">
        ...
        {error && (
          <button onClick={fetchGivingBackHomes} className="mt-6 flex items-center px-6 py-3 bg-gray-800 text-white font-semibold rounded-full hover:bg-gray-700 transition">
  ...
  const imageSrc = data[currentSlide].home_img ? `${baseURL}/${imagePath}` : "https://via.placeholder.com/1200x600?text=Image+Missing";
...
const GivingBackCard: React.FC<{ item: GivingBackData }> = ({ item }) => {
  const [hasImageError, setHasImageError] = useState(false);

  const getFirstImage = () => {
    try {
      const images = JSON.parse(item.image_slider);
      if (Array.isArray(images) && images.length > 0) {
        return `${axiosInstance.defaults.baseURL?.replace(/\/$/, "")}/${images[0].replace(/^\//, "")}`;
      }
    } catch (e) {
      console.error("Failed to parse image_slider JSON:", e);
    }
    return null;
  };
...
const GivingBackSection: React.FC<{ setLoaded: (isLoaded: boolean) => void }> = ({ setLoaded }) => {
  ...
  const fetchGivingBackData = useCallback(async () => {
    ...
    try {
      ...
      setGivingBackData(Array.isArray(response.data.giving_back) ? response.data.giving_back : []);
    } catch (err) {
  ...
  if (error || givingBackData.length === 0) {
    return (
      <div className="w-full py-20 flex flex-col items-center justify-center px-4 text-center">
        ...
        <p className="mt-2 text-gray-600">{error || "There are no Giving Back initiatives to display at the moment."}</p>
        {error && (
          <button onClick={fetchGivingBackData} className="mt-6 flex items-center px-6 py-3 bg-gray-800 text-white font-semibold rounded-full hover:bg-gray-700 transition">
            <ArrowPathIcon className="w-5 h-5 mr-2" />Retry
          </button>
        )}
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl font-bold text-[#ed1c24] inline-flex items-center">
            <HeartIcon className="w-9 h-9 mr-3" />
            Giving Back
          </h2>
          <p className="mt-4 text-lg text-gray-600 max-w-2xl mx-auto">
            Discover how we contribute to our communities and make a positive impact.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mt-12">
          {givingBackData.map((item) => (
            <GivingBackCard key={item.giving_id} item={item} />
          ))}
        </div>
      </div>
    </section>
