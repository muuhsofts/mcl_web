...
const ValuesHomeSlideshow: React.FC = () => {
  ...
  const fetchValueHomes = useCallback(async () => {
    ...
    try {
      ...
      setError("Failed to fetch values sliders.");
      toast.error("Error fetching values sliders.");
    } finally {
      setLoading(false);
    }
  }, []);
...
const ValueCard: React.FC<{ value: Value }> = ({ value }) => {
  ...
  const imageUrl = value.img_file ? `${axiosInstance.defaults.baseURL?.replace(/\/$/, "")}/${value.img_file.replace(/^\//, "")}` : null;
  ...
  return (
    ...
    >
      ...
      <div className="p-8 flex flex-col flex-grow text-black">
        ...
        <p className="text-gray-700 text-base font-medium flex-grow line-clamp-4">{value.description}</p>
      </div>
    </motion.div>
  );
};
...
const ValuesSection: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError("Could not fetch company values data.");
      toast.error("Could not fetch company values data.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  if (error || values.length === 0) {
    return (
      <div className="w-full py-20 flex flex-col items-center justify-center px-4 text-center">
        ...
        <p className="mt-2 text-gray-600">{error || "There are no company values to display at the moment."}</p>
        {error && (
          <button
            onClick={fetchData}
            className="mt-6 flex items-center px-6 py-3 bg-gray-800 text-white font-semibold rounded-full hover:bg-gray-700 transition"
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="mt-4 text-lg text-gray-600 max-w-2xl mx-auto">
            Everything we do is underpinned by our core values, guiding our actions and decisions.
          </p>
        </div>
