...
const AddGivingBackHome = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/giving-back-homes', payload, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      toast.success(response.data.message || 'Giving back home entry created successfully', {
        position: 'top-right',
      });
      setTimeout(() => navigate('/giving-back'), 2000);
    } catch (error: any) {
      const errorMessage = error.response?.data?.error || 'Failed to create giving back home entry';
...
export default AddGivingBackHome;