...
import { useTable, useGlobalFilter, usePagination, Row, CellProps } from 'react-table';
...
const ActionButtons: React.FC<ActionButtonsProps> = ({ giving_back_id, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/giving-back-homes/${giving_back_id}`);
      toast.success('Giving back home entry deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete giving back home entry.', { position: 'top-right' });
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit-giving-back/${giving_back_id}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
...
const ImageModal: React.FC<{ imageUrl: string; onClose: () => void }> = ({ imageUrl, onClose }) => (
  <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50" role="dialog" aria-modal="true" onClick={onClose}>
    <div className="relative bg-white rounded-lg p-4 w-full max-w-3xl" onClick={(e) => e.stopPropagation()}>
      ...
      <img src={imageUrl} alt="Full-size" className="w-full h-auto max-h-[80vh] object-contain rounded" onError={(e) => {
...
export default function GivingBackHome() {
  ...
  const fetchGivingBackHomes = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get<GivingBackHomeData[]>('/api/giving-back-homes');
      setData(response.data);
    } catch (err: unknown) {
      let errorMessage = 'An unknown error occurred while fetching entries.';
      if (axios.isAxiosError(err)) {
        errorMessage = `Failed to fetch entries: ${err.response?.data?.message || err.message}`;
      } else if (err instanceof Error) {
        errorMessage = `An error occurred: ${err.message}`;
      }
      setError(errorMessage);
      toast.error('Failed to fetch giving back home entries.');
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    () => [
      {
        Header: '#',
        id: 'rowIndex',
        Cell: ({ row, flatRows }: { row: Row<GivingBackHomeData>, flatRows: Row<GivingBackHomeData>[] }) => {
      ...
      { Header: 'Heading', accessor: 'heading' },
      {
        ...
        Cell: ({ value }: CellProps<GivingBackHomeData, string | null>) => <DescriptionCell value={value} />,
      },
      {
        Header: 'Image',
        accessor: 'home_img',
        Cell: ({ value }: CellProps<GivingBackHomeData, string | null>) => {
          ...
          return (
            <button onClick={() => setSelectedImage(imageUrl)} className="focus:outline-none" aria-label="View full-size image">
              <img src={imageUrl} alt="Giving back home item" className="h-16 w-16 object-cover rounded cursor-pointer hover:opacity-80 transition" onError={(e) => {
      ...
      },
      {
        Header: 'Created At',
        accessor: 'created_at',
        Cell: ({ value }: CellProps<GivingBackHomeData, string>) => new Date(value).toLocaleDateString(),
      },
      {
        Header: 'Actions',
        accessor: 'giving_back_id',
        Cell: ({ row }: { row: Row<GivingBackHomeData> }) => (
          <ActionButtons giving_back_id={row.original.giving_back_id} onDeletionSuccess={fetchGivingBackHomes} />
        ),
      },
    ] as const,
    [fetchGivingBackHomes]
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('Giving Back Home Entries', 20, 10);
    autoTable(doc, {
    ...
    doc.save('giving_back_home_entries.pdf');
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'GivingBackHomes');
    XLSX.writeFile(workbook, 'giving_back_home_entries.xlsx');
  ...
  if (error && data.length === 0) {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen p-4">
        ...
        <button onClick={fetchGivingBackHomes} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition">Try Again</button>
      </div>
    );
  }
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Giving Back Home Management</h2>
          <Link to="/add/giving-back" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <input value={globalFilter || ''} onChange={(e) => setGlobalFilter(e.target.value)} placeholder="Search entries..." className="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full sm:w-64" />
          ...
          <table {...getTableProps()} className="w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
              ...
              {headerGroups.map((headerGroup) => (
                <tr {...headerGroup.getHeaderGroupProps()}>
                  {headerGroup.headers.map((column) => (
                    <th {...column.getHeaderProps()} className="px-2 sm:px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{column.render('Header')}</th>
                  ))}
                </tr>
              ))}
            </thead>
            <tbody {...getTableBodyProps()} className="divide-y divide-gray-200">
              {page.length > 0 ? (
                page.map((row) => {
                  ...
                  return (
                    <tr {...row.getRowProps()} className="hover:bg-gray-50 transition-colors">
                      {row.cells.map((cell) => (
                        <td {...cell.getCellProps()} className="px-2 sm:px-6 py-4 text-sm text-gray-700">{cell.render('Cell')}</td>
                      ))}
                    </tr>
                  );
                })
                ...
                <tr>
                  <td colSpan={columns.length} className="text-center py-10 text-gray-500">No entries found matching your criteria.</td>
                </tr>
              )}
            </tbody>
          </table>
