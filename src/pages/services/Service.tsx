...
const getFileUrl = (filePath: string | null): string => {
  if (!filePath) return 'https://via.placeholder.com/64x64?text=No+Image';
...
const ActionButtons: React.FC<{ recordId: number; onDelete: () => void }> = React.memo(({ recordId, onDelete }) => {
  ...
  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/services/${recordId}`);
  ...
  return (
    <div className="flex items-center gap-2">
      <Link to={`/services/edit/${recordId}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this service?</p>
...
const Service: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get<{ services: ServiceData[] }>('/api/services');
      setData(response.data.services);
    } catch (err: any) {
      const errorMessage = `Failed to fetch services: ${err.response?.data?.error || err.message}`;
  ...
  const columns: readonly Column<ServiceData>[] = useMemo(() => [
    { Header: '#', Cell: ({ row }: CellProps<ServiceData>) => row.index + 1 },
    { Header: 'Category', accessor: 'service_category' },
    {
      Header: 'Image',
      accessor: 'service_img',
      Cell: ({ value }: CellProps<ServiceData, string | null>) => (
        <button onClick={() => setSelectedImage(getFileUrl(value))} className="focus:outline-none" aria-label="View image">
          <img src={getFileUrl(value)} alt="Service" className="h-16 w-16 object-cover rounded cursor-pointer hover:opacity-80 transition-opacity" />
        </button>
      ),
    },
    { Header: 'Description', accessor: 'description', Cell: ({ value }) => <DescriptionCell value={value} /> },
    {
      ...
      Cell: ({ value }: CellProps<ServiceData, string | null>) =>
        value ? (
          <a href={value} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline font-medium">Visit Link</a>
        ) : (
          <span className="text-gray-500 text-xs italic">No Link</span>
        ),
    },
    { Header: 'Created At', accessor: 'created_at', Cell: ({ value }: CellProps<ServiceData, string>) => new Date(value).toLocaleDateString() },
    { Header: 'Actions', accessor: 'service_id', Cell: ({ row }) => <ActionButtons recordId={row.original.service_id} onDelete={fetchData} /> },
  ], [fetchData]);
  ...
  const exportToPDF = useCallback(() => {
    const doc = new jsPDF();
    doc.text('Service Records', 20, 10);
    autoTable(doc, {
      head: [['#', 'Category', 'Description', 'URL', 'Created At']],
  ...
  const exportToExcel = useCallback(() => {
    const worksheet = XLSX.utils.json_to_sheet(
      data.map((row, index) => ({
        ...
        URL: row.url_link || 'N/A',
        'Created At': new Date(row.created_at).toLocaleDateString(),
      }))
    );
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Services');
    XLSX.writeFile(workbook, 'service_records.xlsx');
  }, [data]);

  if (loading) return <div className="flex justify-center items-center min-h-screen text-lg font-semibold">Loading Services...</div>;
    ...
    <div className="container mx-auto p-4 sm:p-6">
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
              <input value={globalFilter || ''} onChange={(e) => setGlobalFilter(e.target.value)} placeholder="Search services..." className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 w-full sm:w-64" />
            ...
            <div className="overflow-x-auto">
                <table {...getTableProps()} className="w-full border-collapse">
                    <thead className="bg-gray-50">
                      {headerGroups.map((hg) => (<tr {...hg.getHeaderGroupProps()}>{hg.headers.map((col) => <th {...col.getHeaderProps()} className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">{col.render('Header')}</th>)}</tr>))}
                    </thead>
                    <tbody {...getTableBodyProps()}>
                      {page.length ? page.map((row) => { prepareRow(row); return (<tr {...row.getRowProps()} className="hover:bg-gray-50 border-b border-gray-200">{row.cells.map((cell) => <td {...cell.getCellProps()} className="px-6 py-4 whitespace-nowrap">{cell.render('Cell')}</td>)}</tr>);})
                       : (<tr><td colSpan={columns.length} className="text-center py-10 text-gray-500">No services found.</td></tr>)}
                    </tbody>
                </table>
            </div>
            {data.length > 0 && (
