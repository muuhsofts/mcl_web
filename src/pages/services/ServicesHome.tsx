...
interface ActionButtonsProps {
  servicesHomeId: number;
  onDeletionSuccess: () => void;
}

const ActionButtons: React.FC<ActionButtonsProps> = ({ servicesHomeId, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/services-homes/${servicesHomeId}`);
      toast.success('Services Home slider deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete services home slider.', { position: 'top-right' });
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit-serviceshome/${servicesHomeId}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this services home slider?</p>
...
const ImageCell: React.FC<{ value: string | null }> = ({ value }) => {
  ...
  return (
    <a
      ...
      onError={() => console.warn('Error accessing image:', imageUrl)}
    >
      View Image
    </a>
  );
...
export default function ServicesHome() {
  ...
  const fetchSliders = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get('/api/services-homes');
      setData(response.data || []);
    } catch (err: any) {
      const errorMessage = 'Failed to fetch services home sliders: ' + (err.response?.data?.error || err.message || 'Unknown error');
      setError(errorMessage);
      toast.error('Failed to fetch services home sliders.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const columns: readonly Column<ServicesHomeData>[] = useMemo(
    () => [
      {
        Header: '#',
        id: 'rowIndex',
        Cell: ({ row, flatRows }: CellProps<ServicesHomeData>) => {
      ...
      { Header: 'Heading', accessor: 'heading' },
      {
        ...
        Cell: ({ value }: CellProps<ServicesHomeData, string | null>) => <DescriptionCell value={value} />,
      },
      {
        Header: 'Image',
        accessor: 'home_img',
        Cell: ({ value }: CellProps<ServicesHomeData, string | null>) => <ImageCell value={value} />,
      },
      {
        Header: 'Created At',
        accessor: 'created_at',
        Cell: ({ value }: CellProps<ServicesHomeData, string>) => new Date(value).toLocaleDateString(),
      },
      {
        Header: 'Actions',
        accessor: 'services_home_id',
        Cell: ({ row }: CellProps<ServicesHomeData>) => (
          <ActionButtons servicesHomeId={row.original.services_home_id} onDeletionSuccess={fetchSliders} />
        ),
      },
    ],
    [fetchSliders]
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('Services Home Sliders', 20, 10);
    autoTable(doc, {
    ...
    doc.save('services_home_sliders.pdf');
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'ServicesHome');
    XLSX.writeFile(workbook, 'services_home_sliders.xlsx');
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Services Home Management</h2>
          <Link to="/add/services/home" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
            <svg className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
            Create Services Home Slider
          </Link>
        </div>
        ...
        <div className="overflow-x-auto">
          <table {...getTableProps()} className="w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
            ...
            <tbody {...getTableBodyProps()} className="divide-y divide-gray-200">
              {page.length > 0 ? (
                page.map((row: Row<ServicesHomeData>) => {
                  prepareRow(row);
                  return (
                ...
                <tr><td colSpan={columns.length} className="text-center py-10 text-gray-500">No services home sliders found matching your criteria.</td></tr>
              )}
            </tbody>
          </table>
        </div>
