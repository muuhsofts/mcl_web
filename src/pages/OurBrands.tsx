...
import { InformationCircleIcon, ArrowUpRightIcon, InboxIcon } from "@heroicons/react/24/outline";
...
// --- BrandCard COMPONENT (per-card state for "Show More") ---
const BrandCard: React.FC<{ brand: BrandData; variants: Variants }> = ({ brand, variants }) => {
  const [isExpanded, setIsExpanded] = useState(false); // 👈 unique per card – ensures only this card expands
      ...
      <div className="relative bg-white shadow-lg rounded-xl flex flex-col group transition-all duration-300 border border-transparent group-hover:border-[#0072bc] -mt-16 flex-grow">
        <div className="p-6 pt-20 flex flex-col flex-grow">
          ...
          <div className="mt-auto pt-4 flex flex-wrap items-center justify-between gap-3">
            {/* "Read More" link – always visible, opens individual brand page */}
            <Link
              to={targetUrl}
            ...
            {/* "Show More / Show Less" button – only appears for long descriptions,
                toggles only this card's description */}
            {isLongDescription && (
              <button
                onClick={() => setIsExpanded(!isExpanded)}
                className="text-sm font-semibold text-[#0072bc] hover:text-[#003459] transition-colors"
              >
  ...
  const fetchBrands = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(brandsData)) {
        // Sort by brand_id descending to show newest first
        ...
        <section className="py-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              ...
              <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="mt-4 text-lg text-gray-600 max-w-3xl mx-auto"></motion.p>
            </div>
