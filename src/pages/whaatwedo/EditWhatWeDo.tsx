...
const EditWhatWeDo: React.FC = () => {
  const navigate = useNavigate();
  const { what_we_do_id } = useParams<{ what_we_do_id: string }>();
  ...
  useEffect(() => {
    const fetchWhatWeDoHome = async () => {
      ...
      try {
        const response = await axiosInstance.get<WhatWeDoHomeData>(`/api/what-we-do-homes/${what_we_do_id}`);
        setFormData({
        ...
        toast.error('Failed to fetch what we do entry');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.post(`/api/what-we-do-homes/${what_we_do_id}`, payload, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      toast.success(response.data.message || 'What we do entry updated successfully');
