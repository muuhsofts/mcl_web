...
const ActionButtons: React.FC<ActionButtonsProps> = ({ what_we_do_id, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/what-we-do-homes/${what_we_do_id}`);
      toast.success('What we do entry deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err: any) {
      toast.error('Failed to delete what we do entry.', { position: 'top-right' });
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link
        to={`/edit/what-we-do/${what_we_do_id}`}
      ...
      {showConfirm && (
        ...
        >
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this what we do entry?</p>
...
const WhatWeDo: React.FC = () => {
  ...
  const fetchWhatWeDoHomes = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get<WhatWeDoHomeData[]>('/api/what-we-do-homes');
      setData(response.data);
    } catch (err: any) {
      const errorMessage =
        'Failed to fetch what we do entries: ' + (err.response?.data?.message || err.message || 'Unknown error');
      setError(errorMessage);
      toast.error('Failed to fetch what we do entries.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const columns: readonly Column<WhatWeDoHomeData>[] = useMemo(
    () => [
      {
        Header: '#',
        id: 'rowIndex',
        Cell: ({ row, flatRows }: CellProps<WhatWeDoHomeData>) =>
          flatRows.findIndex((flatRow) => flatRow.original.what_we_do_id === row.original.what_we_do_id) + 1,
      },
      { Header: 'Heading', accessor: 'heading' },
      {
        Header: 'Description',
        accessor: 'description',
        Cell: ({ value }: CellProps<WhatWeDoHomeData, string | null>) => <DescriptionCell value={value} />,
      },
      {
        Header: 'Image',
        accessor: 'home_img',
        Cell: ({ value }: CellProps<WhatWeDoHomeData, string | null>) => {
      ...
      },
      {
        Header: 'Created At',
        accessor: 'created_at',
        Cell: ({ value }: CellProps<WhatWeDoHomeData, string>) => new Date(value).toLocaleDateString(),
      },
      {
        Header: 'Actions',
        id: 'actions',
        Cell: ({ row }: CellProps<WhatWeDoHomeData>) => (
          <ActionButtons
            what_we_do_id={row.original.what_we_do_id}
            onDeletionSuccess={fetchWhatWeDoHomes}
          />
        ),
      },
    ],
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('What We Do Entries', 20, 10);
    autoTable(doc, {
    ...
    doc.save('what_we_do_entries.pdf');
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'WhatWeDoHomes');
    XLSX.writeFile(workbook, 'what_we_do_entries.xlsx');
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <input
            ...
            placeholder="Search what we do entries..."
              ...
              {page.length > 0 ? (
                ...
                <tr>
                  <td colSpan={columns.length} className="text-center py-10 text-gray-500">
                    No what we do entries found matching your criteria.
                  </td>
                </tr>
              )}
            </tbody>
