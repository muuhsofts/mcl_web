...
const AddSustainabilityHome: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/sustainability-homes', payload, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      toast.success(response.data.message || 'Sustainability home entry created successfully', {
        position: 'top-right',
      });
      setTimeout(() => navigate('/sustainability/home'), 2000);
    } catch (error: any) {
      const errorMessage = error.response?.data?.error || 'Failed to create sustainability home entry';
...
export default AddSustainabilityHome;