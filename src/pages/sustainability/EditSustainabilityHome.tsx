...
// Interface for form error messages
...
const EditSustainabilityHome: React.FC = () => {
  const navigate = useNavigate();
  const { sustainability_home_id } = useParams<{ sustainability_home_id: string }>();
  ...
  useEffect(() => {
    const fetchSustainabilityHome = async () => {
      if (!sustainability_home_id) {
        toast.error('Sustainability Home ID is missing.');
        navigate('/sustainability/home');
        return;
      }
      try {
        const response = await axiosInstance.get<SustainabilityHomeData>(`/api/sustainability-homes/${sustainability_home_id}`);
        setFormData({
        ...
        toast.error('Failed to fetch sustainability home entry');
        console.error("Fetch error:", error);
        navigate('/sustainability/home');
      }
    };

    fetchSustainabilityHome();
  }, [sustainability_home_id, navigate]);
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.home_img) {
      // REFINEMENT: The file size check has been completely removed.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.post(`/api/sustainability-homes/${sustainability_home_id}`, payload, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      toast.success(response.data.message || 'Sustainability home entry updated successfully');
      setFormData((prev) => ({ ...prev, home_img: null }));
      setCurrentImage(response.data.sustainabilityHome?.home_img || null);
      setTimeout(() => navigate('/sustainability/home'), 2000);
    } catch (error: any) {
      const errorMessage = error.response?.data?.error || 'Failed to update sustainability home entry';
    ...
    <div className="p-4 sm:p-6 lg:p-8 w-full min-h-screen">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 lg:p-8 w-full">
        <h2 className="text-xl sm:text-2xl lg:text-3xl font-semibold text-gray-800 mb-6">
          Edit Sustainability Home Entry
        </h2>
          ...
          <div>
            ...
            {displayImageUrl && (
              <div className="my-2">
                ...
                <img
                  src={displayImageUrl}
                  alt="Current Sustainability Home"
            ...
            {/* REFINEMENT: Helper text updated to remove size limit. */}
            <p className="mt-1 text-xs text-gray-500">Allowed types: JPG, PNG, GIF.</p>
          </div>
          <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4">
            <button
              type="button"
              onClick={() => navigate('/sustainability/home')}
...
export default EditSustainabilityHome;