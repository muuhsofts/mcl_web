...
const EditAbout: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAbout = async () => {
      if (!aboutId) {
        toast.error('About ID is missing.');
        navigate('/about');
        return;
      }
      try {
        const response = await axiosInstance.get(`/api/about/${aboutId}`);
        setFormData({
        ...
        toast.error('Failed to fetch about entry');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // For Laravel PUT/PATCH with FormData, you might need to append _method
    // payload.append('_method', 'PUT');

    try {
      const response = await axiosInstance.post(`/api/about/${aboutId}/update`, payload, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      toast.success(response.data.message || 'About entry updated successfully');
      if (response.data.updated_image_path) {
         setCurrentImage(response.data.updated_image_path);
  ...
  return (
    // --- MODIFICATION HERE: Removed max-w-4xl and mx-auto (mx-auto is not needed for w-full) ---
    <div className="p-4 sm:p-6 lg:p-8 w-full min-h-screen"> {/* Added min-h-screen for full height coverage too if desired */}
      <ToastContainer position="top-right" autoClose={3000} style={{ top: '70px', zIndex: 9999 }} />
      {/* The inner card will take the full width of its parent (which is now the full screen width minus padding) */}
