...
const ActionButtons: React.FC<ActionButtonsProps> = ({ aboutId, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/about/${aboutId}`);
      toast.success('About entry deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete about entry.', { position: 'top-right' });
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit-about/${aboutId}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this about entry?</p>
...
export default function About() {
  ...
  const fetchAbout = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get<AboutData[]>('/api/about');
      setData(response.data);
    } catch (err: any) {
      const errorMessage = 'Failed to fetch about entries: ' + (err.response?.data?.message || err.message || 'Unknown error');
  ...
  const columns: readonly Column<AboutData>[] = useMemo(
    () => [
      {
        Header: '#',
        id: 'rowIndex',
        Cell: ({ row, flatRows }: CellProps<AboutData>) => {
      ...
      },
      {
        Header: 'Image',
        accessor: 'home_img',
        Cell: ({ value }: CellProps<AboutData, string | null>) => {
      ...
      },
      {
        Header: 'Created At',
        accessor: 'created_at',
        Cell: ({ value }: CellProps<AboutData, string>) => new Date(value).toLocaleDateString(),
      },
      {
        Header: 'Actions',
        accessor: 'about_id',
        Cell: ({ row }: CellProps<AboutData>) => (
          <ActionButtons aboutId={row.original.about_id} onDeletionSuccess={fetchAbout} />
        ),
      },
    ],
    [fetchAbout]
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'About');
    XLSX.writeFile(workbook, 'about_entries.xlsx');
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">About Management</h2>
          <Link to="/add/about" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
          ...
          <table {...getTableProps()} className="w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
            ...
            <tbody {...getTableBodyProps()} className="divide-y divide-gray-200">
              {page.length > 0 ? (
                ...
                <tr><td colSpan={columns.length} className="text-center py-10 text-gray-500">No about entries found matching your criteria.</td></tr>
              )}
            </tbody>
          </table>
        </div>
