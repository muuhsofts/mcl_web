...
const ActionButtons: React.FC<ActionButtonsProps> = ({ id, onDeletionSuccess }) => {
  ...
  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/about-mwananchi/${id}`);
  ...
  return (
    <div className="flex items-center gap-2">
      <Link to={`/edit/aboutmwanachi/${id}`} className="p-1 text-blue-500 hover:text-blue-600">
...
const VideoLinkCell: React.FC<{ value: string | null }> = ({ value }) => {
...
const PdfFileCell: React.FC<{ value: string | null }> = ({ value }) => {
...
const AboutMwananchi: React.FC = () => {
  const [data, setData] = useState<AboutMwananchiData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const fetchAboutMwananchi = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await axiosInstance.get<{ records: AboutMwananchiData[] }>('/api/about-mwananchi');
  ...
  const columns: readonly Column<AboutMwananchiData>[] = useMemo(
    () => [
      {
        Header: '#',
        id: 'rowIndex',
        Cell: ({ row, flatRows }: CellProps<AboutMwananchiData>) => flatRows.indexOf(row) + 1,
      },
      { Header: 'Category', accessor: 'category' },
      { Header: 'Description', accessor: 'description', Cell: DescriptionCell },
      { Header: 'Video', accessor: 'video_link', Cell: VideoLinkCell },
      { Header: 'PDF', accessor: 'pdf_file', Cell: PdfFileCell },
      {
        Header: 'Created At',
        accessor: 'created_at',
        Cell: ({ value }: CellProps<AboutMwananchiData, string>) => new Date(value).toLocaleDateString(),
      },
      { Header: 'Actions', accessor: 'id', Cell: ({ row }) => (
        <ActionButtons id={row.original.id} onDeletionSuccess={fetchAboutMwananchi} />
      )},
    ],
    [fetchAboutMwananchi]
  );
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('About Mwananchi Entries', 20, 10);
    autoTable(doc, {
      head: [['#', 'Category', 'Description', 'Video Link', 'PDF File', 'Created At']],
    ...
    doc.save('about_mwananchi_entries.pdf');
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      data.map((row, index) => ({
        ...
        'Video Link': row.video_link || 'N/A',
        'PDF File': row.pdf_file || 'N/A',
        'Created At': new Date(row.created_at).toLocaleDateString(),
      }))
    );
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'AboutMwananchi');
    XLSX.writeFile(workbook, 'about_mwananchi_entries.xlsx');
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">About Mwananchi Management</h2>
          <Link
            to="/add/about-mwananchi"
        ...
        <div className="overflow-x-auto">
          <table {...getTableProps()} className="w-full border border-solid border-[#0A51A1] rounded-lg">
            ...
            <tbody {...getTableBodyProps()} className="divide-y divide-gray-200">
              {page.length > 0 ? (
                page.map((row: Row<AboutMwananchiData>) => {
                  prepareRow(row);
                  return (
