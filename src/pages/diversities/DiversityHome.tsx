...
const ActionButtons: React.FC<ActionButtonsProps> = ({ dhomeId, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/diversity-home/${dhomeId}`);
      toast.success('Diversity home entry deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete diversity home entry.', { position: 'top-right' });
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit-diversity-home/${dhomeId}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this diversity home entry?</p>
...
export default function DiversityHome() {
  ...
  const fetchDiversityHome = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await axiosInstance.get<DiversityHomeData[]>('/api/diversity-home');
      setData(response.data);
    } catch (err: any) {
      const errorMessage = 'Failed to fetch diversity home entries: ' + (err.response?.data?.message || err.message || 'Unknown error');
      setError(errorMessage);
      toast.error('Failed to fetch diversity home entries.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const columns: readonly Column<DiversityHomeData>[] = useMemo(
    () => [
      {
        Header: '#',
        id: 'rowIndex',
        Cell: ({ row, flatRows }: CellProps<DiversityHomeData>) => {
      ...
      { Header: 'Heading', accessor: 'heading' },
      {
        ...
        Cell: ({ value }: CellProps<DiversityHomeData, string>) => <DescriptionCell value={value} />,
      },
      {
        Header: 'Image',
        accessor: 'home_img',
        Cell: ({ value }: CellProps<DiversityHomeData, string | null>) => {
      ...
      },
      {
        Header: 'Created At',
        accessor: 'created_at',
        Cell: ({ value }: CellProps<DiversityHomeData, string>) => new Date(value).toLocaleDateString(),
      },
      {
        Header: 'Actions',
        accessor: 'dhome_id',
        Cell: ({ row }: CellProps<DiversityHomeData>) => (
          <ActionButtons dhomeId={row.original.dhome_id} onDeletionSuccess={fetchDiversityHome} />
        ),
      },
    ],
    [fetchDiversityHome]
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('Diversity Home Entries', 20, 10);
    autoTable(doc, {
    ...
    doc.save('diversity_home_entries.pdf');
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'DiversityHome');
    XLSX.writeFile(workbook, 'diversity_home_entries.xlsx');
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Diversity and Inclusion Home Management</h2>
          <Link to="/add/diversity-and-inclusion" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
            <svg className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
            Create Diversity and Inclusion Home
          </Link>
        </div>
        ...
        <div className="overflow-x-auto">
          <table {...getTableProps()} className="w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
            ...
            <tbody {...getTableBodyProps()} className="divide-y divide-gray-200">
              {page.length > 0 ? (
                page.map((row: Row<DiversityHomeData>) => {
                  prepareRow(row);
                  return (
                ...
                <tr><td colSpan={columns.length} className="text-center py-10 text-gray-500">No diversity home entries found matching your criteria.</td></tr>
              )}
            </tbody>
          </table>
        </div>
