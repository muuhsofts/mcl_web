...
const EditDiversityHome: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDiversityHome = async () => {
      if (!dhomeId) {
        toast.error('Diversity home ID is missing.');
        navigate('/diversity-and-inclusion');
        return;
      }
      try {
        const response = await axiosInstance.get(`/api/diversity-home/${dhomeId}`);
        setFormData({
        ...
        toast.error('Failed to fetch diversity home entry');
        console.error("Fetch error:", error);
        navigate('/diversity-home');
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.post(`/api/diversity-home/${dhomeId}/update`, payload, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      toast.success(response.data.message || 'Diversity home entry updated successfully');
      if (response.data.updated_image_path) {
        setCurrentImage(response.data.updated_image_path);
        setFormData(prev => ({ ...prev, home_img: null }));
      }
      setTimeout(() => navigate('/diversity-and-inclusion'), 2000);
    } catch (error: any) {
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 lg:p-8 w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4">
            <button
              type="button"
              onClick={() => navigate('/diversity-and-inclusion')}
...
export default EditDiversityHome;