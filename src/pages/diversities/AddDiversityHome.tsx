...
const AddDiversityHome: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/diversity-home', payload, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      toast.success(response.data.message || 'Diversity home entry created successfully', {
        position: 'top-right',
      });
      setTimeout(() => navigate('/diversity-and-inclusion'), 2000);
    } catch (error: any) {
      const errorMessage = error.response?.data?.message || 'Failed to create diversity home entry';
...
export default AddDiversityHome;