  ...
  if (error || data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh] text-center p-6 bg-gray-800">
        ...
        {error && (
          <button onClick={fetchSlides} className="mt-6 flex items-center px-6 py-3 bg-gray-800 text-white font-semibold rounded-full hover:bg-gray-700 transition">
    ...
    <section className="relative min-h-[80vh] w-full overflow-hidden bg-gray-800">
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <img
            src={data[currentSlide].mcl_home_img ? `${axiosInstance.defaults.baseURL?.replace(/\/$/, "")}/${data[currentSlide].mcl_home_img!.replace(/^\//, "")}` : "https://via.placeholder.com/1200x600?text=Image+Missing"}
  ...
  return (
    ...
    >
      ...
      <div className="p-8 flex flex-col flex-grow text-black">
        ...
        <p className="text-gray-700 text-base font-medium flex-grow line-clamp-4">{group.description}</p>
        <div className="mt-6">
          {group.weblink && (
            <Link
              to={group.weblink}
...
const MCLGroupsSection: React.FC = () => {
  ...
  const fetchGroups = useCallback(async () => {
    ...
    setError(null);
    try {
      // Fetch all groups from the backend without any limit
      ...
      setError("Could not fetch company groups.");
  ...
  if (error || groups.length === 0) {
    return (
      <div className="w-full py-20 flex flex-col items-center justify-center px-4 text-center">
        ...
        <h3 className="mt-4 text-2xl font-bold text-gray-800">{error ? "Failed to Load Content" : "No Groups Available"}</h3>
        <p className="mt-2 text-gray-600">{error || "There are no company groups to display at the moment."}</p>
        {error && (
          <button onClick={fetchGroups} className="mt-6 flex items-center px-6 py-3 bg-gray-800 text-white font-semibold rounded-full hover:bg-gray-700 transition">
            <ArrowPathIcon className="w-5 h-5 mr-2" />Retry
          </button>
        )}
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="mt-4 text-lg text-gray-600 max-w-2xl mx-auto">
            Discover the diverse groups under our company umbrella.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mt-12">
          {groups.map((group) => (
            <MCLGroupCard key={group.mcl_id} group={group} />
          ))}
        </div>
      </div>
    </section>
