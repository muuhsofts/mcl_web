...
const ActionButtons: React.FC<ActionButtonsProps> = ({ brand_id, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/brands/${brand_id}`);
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit/brand/${brand_id}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this brand?</p>
...
export default function Brands() {
  ...
  const fetchBrands = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.error || 'Failed to fetch brands.';
    ...
    () => [
      ...
      },
      {
        ...
        Cell: ({ value }: { value: string | null }) => {
          ...
          return (
            <button onClick={() => handleImageClick(imageUrl)} className="focus:outline-none">
              <img
                src={imageUrl}
                alt="Brand"
      ...
      },
      {
        Header: 'Actions',
        accessor: 'brand_id',
        Cell: ({ row }: { row: Row<BrandData> }) => (
          <ActionButtons brand_id={row.original.brand_id} onDeletionSuccess={fetchBrands} />
        ),
      },
    ],
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Brands Management</h2>
          <Link to="/add/brand" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
          ...
          <table {...getTableProps()} className="w-full">
            ...
            <tbody {...getTableBodyProps()} className="divide-y divide-gray-200">
              {page.length > 0 ? (
                ...
                <tr><td colSpan={columns.length} className="text-center py-10 text-gray-500">No brands found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {data.length > pageSize && (
          <div className="flex flex-col sm:flex-row justify-between items-center mt-6 gap-4">
            ...
            <span className="text-sm text-gray-700">Page <strong>{pageIndex + 1} of {pageOptions.length}</strong></span>
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-4 w-full max-w-3xl relative">
            ...
            <img
              ...
              onError={(e) => {
                (e.currentTarget as HTMLImageElement).src = 'https://via.placeholder.com/400x400?text=Error';
              }}
            />
          </div>
        </div>
