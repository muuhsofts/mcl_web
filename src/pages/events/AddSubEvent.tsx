...
const AddSubEvent: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/sub-events', payload, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      toast.success(response.data.message || 'Sub-event created successfully!', {
        position: 'top-right',
      });
      setTimeout(() => navigate('/sub-events'), 2000);
    } catch (error: any) {
      const errorMessage = error.response?.data?.error || 'Failed to create sub-event.';
      const backendErrors: Partial<Errors> = error.response?.data?.errors || {};
      setErrors((prev: Errors) => ({
        ...prev,
        event_id: backendErrors.event_id || '',
        sub_category: backendErrors.sub_category || '',
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 lg:p-8 w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-6" noValidate>
          <div>
            ...
            {errors.event_id && (
              <p id="event_id-error" className="mt-1 text-sm text-red-500">
                {errors.event_id}
              </p>
            )}
          </div>
          <div>
            ...
            <input
              ...
              maxLength={255}
              aria-invalid={!!errors.sub_category}
              aria-describedby={errors.sub_category ? 'sub_category-error' : undefined}
              required
            />
            {errors.sub_category && (
              <p id="sub_category-error" className="mt-1 text-sm text-red-500">
                {errors.sub_category}
              </p>
            )}
          </div>
