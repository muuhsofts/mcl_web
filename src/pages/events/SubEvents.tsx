...
const ActionButtons: React.FC<ActionButtonsProps> = ({ subEventId, subCategory, onDeleteSuccess }) => {
  ...
  const handleDelete = useCallback(async () => {
    setIsDeleting(true);
    try {
      await axiosInstance.delete(`/api/sub-events/${subEventId}`);
  ...
  }, [subEventId, onDeleteSuccess]);

  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit/sub-event/${subEventId}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label={`Edit ${subCategory}`}>
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete the sub-event "{subCategory}"?</p>
...
const SubEvents: React.FC = () => {
  const [data, setData] = useState<SubEventData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedMedia, setSelectedMedia] = useState<{ url: string; type: MediaType } | null>(null);

  const mapSubEventData = useCallback((sub_events: SubEventApiResponse['sub_events']['data']): SubEventData[] => {
    return sub_events.map((sub_event) => ({
      subEventId: sub_event.subevent_id,
      eventId: sub_event.event_id,
      eventCategory: sub_event.event?.event_category || 'N/A',
      subCategory: sub_event.sub_category,
      description: sub_event.description ?? null,
      imgFile: sub_event.img_file ?? null,
      videoLink: sub_event.video_link ?? null,
      createdAt: sub_event.created_at,
    }));
  }, []);

  const fetchSubEvents = useCallback(async (page: number = 1, pageSize: number = 10) => {
    setLoading(true);
    setError(null);
    try {
      const response = await axiosInstance.get<SubEventApiResponse>(`/api/sub-events?page=${page}&per_page=${pageSize}`);
      if (!response.data.sub_events.data) {
        throw new Error('Invalid API response: sub_events data is missing');
      }
      setData(mapSubEventData(response.data.sub_events.data));
    } catch (err: any) {
      const errorMessage = err.response?.data?.error || err.message || 'Failed to fetch sub-events';
  ...
  const columns = useMemo<Column<SubEventData>[]>(
    () => [
        ...
        Cell: ({ value }: { value: string | null }) => {
          ...
          return (
            <button
              ...
              aria-label="View sub-event video"
            >
              View Video
            </button>
          );
      ...
      { Header: 'Created At', accessor: 'createdAt', Cell: ({ value }) => new Date(value).toLocaleDateString() },
      {
        ...
        accessor: 'subEventId',
        Cell: ({ row }) => (
          <ActionButtons subEventId={row.original.subEventId} subCategory={row.original.subCategory} onDeleteSuccess={() => fetchSubEvents()} />
        ),
      },
    ],
    [fetchSubEvents]
  ...
  } = useTable<SubEventData>({ columns, data, initialState: { pageIndex: 0, pageSize: 10 }, pageCount: -1 }, useGlobalFilter, usePagination);

  const exportToPDF = useCallback(() => {
    const doc = new jsPDF();
    doc.text('Sub-Event Records', 20, 10);
    autoTable(doc, {
      head: [['#', 'Event Category', 'Sub-Category', 'Description', 'Created At']],
    ...
    doc.save('sub_event_records.pdf');
  ...
  const exportToExcel = useCallback(() => {
    const worksheet = XLSX.utils.json_to_sheet(
      data.map((row, index) => ({
        '#': index + 1,
        'Event Category': row.eventCategory,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'SubEvents');
    XLSX.writeFile(workbook, 'sub_event_records.xlsx');
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Sub-Event Management</h2>
          <Link
            to="/add/sub-event"
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              aria-label="Export sub-events to PDF"
            >
              Export PDF
            </button>
            <button
              ...
              aria-label="Export sub-events to Excel"
            >
              Export Excel
            </button>
          </div>
