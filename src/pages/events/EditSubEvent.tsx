...
const EditSubEvent: React.FC = () => {
  ...
  const { subevent_id } = useParams<{ subevent_id: string }>();
  ...
  useEffect(() => {
    const fetchSubEvent = async () => {
      if (!subevent_id) {
        toast.error('Sub-Event ID is missing from the URL.');
        navigate('/sub-events');
        return;
      }
      try {
        const response = await axiosInstance.get(`/api/sub-events/${subevent_id}`);
        const subEventData = response.data.sub_event;
        setFormData({
          event_id: subEventData?.event_id?.toString() || '',
          sub_category: subEventData?.sub_category || '',
          description: subEventData?.description || '',
          img_file: null,
          video_link: subEventData?.video_link || '',
        });
        setCurrentImage(subEventData?.img_file || null);
        setCurrentVideoLink(subEventData?.video_link || null);
      } catch (error) {
        toast.error('Failed to fetch the sub-event record.');
    ...
    const fetchDropdownData = async () => {
      try {
        const response = await axiosInstance.get<DropdownData>('/api/events/dropdown-data');
  ...
  const validateForm = (): boolean => {
    ...
    } else if (formData.sub_category.length > 255) {
      newErrors.sub_category = 'Sub-category must not exceed 255 characters';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    payload.append('event_id', formData.event_id);
    ...
    try {
      const response = await axiosInstance.post(`/api/sub-events/${subevent_id}`, payload, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });

      toast.success(response.data.message || 'Sub-event updated successfully!');
      setTimeout(() => navigate('/sub-events'), 2000);
    } catch (error: any) {
      const errorMessage = error.response?.data?.error || 'Failed to update sub-event.';
        ...
        <form onSubmit={handleSubmit} className="space-y-6" noValidate>
          <div>
            ...
            <select
              ...
              className={`${inputBaseClasses} ${errors.event_id ? inputErrorClasses : inputBorderClasses}`}
            ...
            >
              ...
              {eventOptions.map((option) => (
                <option key={option.event_id} value={option.event_id}>
                  {option.event_category}
                </option>
              ))}
            </select>
            {errors.event_id && <p id="event_id-error" className="mt-1 text-sm text-red-500">{errors.event_id}</p>}
          </div>
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className={`${inputBaseClasses} ${errors.sub_category ? inputErrorClasses : inputBorderClasses}`}
          ...
          <div>
            <label htmlFor="img_file" className="block text-sm font-medium text-gray-700">
              Sub-Event Image <span className="text-gray-500">(optional, replace current)</span>
            </label>
            {displayImageUrl && (
              <div className="my-2">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className={`${inputBaseClasses} ${errors.video_link ? inputErrorClasses : inputBorderClasses}`}
