...
const ActionButtons: React.FC<ActionButtonsProps> = ({ companyId, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/companies/${companyId}`);
      toast.success('Company entry deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete company entry.', { position: 'top-right' });
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this company entry?</p>
...
export default function CompHome() {
  ...
  const fetchCompanies = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get<CompanyData[]>('/api/companies');
      setData(response.data);
    } catch (err: any) {
      const errorMessage = 'Failed to fetch company entries: ' + (err.response?.data?.message || err.message || 'Unknown error');
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('Company Entries', 20, 10);
    autoTable(doc, {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, 'company_entries.xlsx');
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Company Home Management</h2>
          <Link to="/add/company/home" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
