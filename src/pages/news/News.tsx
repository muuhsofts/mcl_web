...
// Corrected: Removed 'Row' as it was unused.
...
/** Defines the structure of a single news record. */
interface NewsData {
  news_id: number;
  category: string;
...
/** Defines the shape of the API response for fetching news. */
...
/**
 * Constructs a full, absolute URL for a given file path.
 * @param filePath The relative path from the API.
...
/**
 * Renders Edit and Delete buttons for a table row, with a confirmation modal for deletion.
 */
const ActionButtons: React.FC<ActionButtonsProps> = ({ newsId, category, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/news/${newsId}`);
      toast.success('News record deleted successfully!');
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete news record.');
    } finally {
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit/news/${newsId}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label={`Edit ${category}`}>
      ...
      <button onClick={() => setShowConfirm(true)} className="p-1 text-red-500 hover:text-red-600" aria-label={`Delete ${category}`}>
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete the news record for "{category}"?</p>
...
/**
 * A cell renderer that truncates long descriptions and provides a "Read More" toggle.
 */
...
/**
 * A modal component to display a full-size image when a thumbnail is clicked.
 */
const ImageModal: React.FC<{ imageUrl: string; onClose: () => void }> = ({ imageUrl, onClose }) => (
  <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50" onClick={onClose}>
    <div className="relative bg-white rounded-lg p-4 w-full max-w-3xl" onClick={(e) => e.stopPropagation()}>
      ...
      <img src={imageUrl} alt="Full-size view" className="w-full h-auto max-h-[80vh] object-contain rounded" onError={(e) => { e.currentTarget.src = 'https://via.placeholder.com/300x300?text=Error'; }}/>
    </div>
  </div>
);
...
/**
 * The main component for the News Management page.
 * Displays news records in a filterable, sortable, and paginated table.
 */
const News: React.FC = () => {
  ...
  const fetchNews = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get<NewsApiResponse>('/api/news');
      setData(response.data.news || []);
    } catch (err: any) {
      const errorMessage = 'Failed to fetch news records. ' + (err.response?.data?.error || err.message);
      setError(errorMessage);
      toast.error('Failed to fetch news records.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const columns = useMemo<readonly Column<NewsData>[]>(
    () => [
      { Header: '#', Cell: ({ row }: CellProps<NewsData>) => <span>{row.index + 1}</span> },
      { Header: 'Category', accessor: 'category' },
      { Header: 'Description', accessor: 'description', Cell: ({ value }) => <DescriptionCell value={value} /> },
      {
        Header: 'Image',
        accessor: 'news_img',
        Cell: ({ value }: CellProps<NewsData, string | null>) => {
          ...
          return (
            <button onClick={() => setSelectedImage(imageUrl)} className="focus:outline-none" aria-label="View full-size image">
              <img src={imageUrl} alt="News thumbnail" className="h-16 w-16 object-cover rounded cursor-pointer hover:opacity-80 transition" onError={(e) => { e.currentTarget.src = 'https://via.placeholder.com/64x64?text=Error'; }} />
            </button>
          );
        },
      },
      {
        ...
        accessor: 'pdf_file',
        Cell: ({ value }: CellProps<NewsData, string | null>) => {
          const pdfUrl = constructFileUrl(value);
          if (!pdfUrl) return <span className="text-gray-500 text-xs">No PDF</span>;
          return <a href={pdfUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-600 hover:underline text-sm font-medium">View PDF</a>;
        },
      },
      { Header: 'Created At', accessor: 'created_at', Cell: ({ value }) => new Date(value).toLocaleDateString() },
      { Header: 'Actions', accessor: 'news_id', Cell: ({ row }) => <ActionButtons newsId={row.original.news_id} category={row.original.category} onDeletionSuccess={fetchNews} /> },
    ],
    [fetchNews]
  );

  const tableInstance = useTable({ columns, data, initialState: { pageIndex: 0, pageSize: 10 } }, useGlobalFilter, usePagination);
  const { getTableProps, getTableBodyProps, headerGroups, page, prepareRow, canPreviousPage, canNextPage, pageOptions, nextPage, previousPage, setPageSize, setGlobalFilter, state: { pageIndex, pageSize, globalFilter } } = tableInstance;

  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('News Records', 20, 10);
    autoTable(doc, {
      head: [['#', 'Category', 'Description', 'Created At']],
      body: data.map((row, index) => [index + 1, row.category, row.description || 'N/A', new Date(row.created_at).toLocaleDateString()]),
    });
    doc.save('news_records.pdf');
    toast.success('PDF exported successfully!');
  };

  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(data.map((row, index) => ({ '#': index + 1, Category: row.category, Description: row.description || 'N/A', 'Created At': new Date(row.created_at).toLocaleDateString() })));
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'News');
    XLSX.writeFile(workbook, 'news_records.xlsx');
  ...
  if (error && data.length === 0) return (
    <div className="flex flex-col justify-center items-center min-h-screen p-4 text-center">
      ...
      <button onClick={fetchNews} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition">Try Again</button>
    </div>
  );
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">News Management</h2>
          <Link to="/add/news" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <input value={globalFilter || ''} onChange={(e) => setGlobalFilter(e.target.value)} placeholder="Search records..." className="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full sm:w-auto" />
          ...
          <table {...getTableProps()} className="w-full">
            ...
            <tbody {...getTableBodyProps()} className="divide-y divide-gray-200">
              {page.length > 0 ? (
                page.map((row) => {
                  prepareRow(row);
                  return <tr {...row.getRowProps()} className="hover:bg-gray-50 transition-colors">{row.cells.map((cell) => <td {...cell.getCellProps()} className="px-4 py-4 align-top">{cell.render('Cell')}</td>)}</tr>;
                })
              ) : (
