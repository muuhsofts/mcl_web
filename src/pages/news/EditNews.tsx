...
const EditNews: React.FC = () => {
  ...
  const [currentReadMoreUrl, setCurrentReadMoreUrl] = useState<string | null>(null);
    ...
    const fetchNews = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          news_img: null,
          pdf_file: null,
          read_more_url_lnk: newsData?.read_more_url_lnk || '',
        });
        setCurrentImage(newsData?.news_img || null);
        setCurrentPdf(newsData?.pdf_file || null);
        setCurrentReadMoreUrl(newsData?.read_more_url_lnk || null);
      } catch (error) {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.read_more_url_lnk) {
      ...
      if (!urlPattern.test(formData.read_more_url_lnk)) {
        newErrors.read_more_url_lnk = 'Please enter a valid URL';
      }
            ...
            {currentReadMoreUrl && (
              <div className="my-2">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-blue-500 hover:text-blue-600 hover:underline text-sm font-medium break-all"
                >
                  {currentReadMoreUrl}
                </a>
              </div>
