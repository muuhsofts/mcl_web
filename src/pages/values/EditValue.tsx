...
const EditValue = () => {
  ...
  const { value_id } = useParams<{ value_id: string }>();
  ...
  useEffect(() => {
    const fetchValue = async () => {
      if (!value_id) {
        toast.error('Value ID is missing.');
        navigate('/values');
        return;
      }
      try {
        const response = await axiosInstance.get(`/api/values/${value_id}`);
        ...
        setCurrentImage(value.img_file);
      } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.post(`/api/values/${value_id}`, payload, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      toast.success(response.data.message || 'Value updated successfully');
      setFormData((prev) => ({ ...prev, img_file: null }));
      setCurrentImage(response.data.value?.img_file || null);
      setTimeout(() => navigate('/values'), 2000);
    } catch (error: any) {
      const errorMessage = error.response?.data?.error || 'Failed to update value';
