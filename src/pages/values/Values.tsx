...
// Define the ValueData interface
interface ValueData {
  value_id: number;
  category: string;
...
const ActionButtons: React.FC<ActionButtonsProps> = ({ value_id, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/values/${value_id}`);
      toast.success('Value deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete value.', { position: 'top-right' });
      ...
      {showConfirm && (
        ...
        >
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this value?</p>
...
export default function Values() {
  ...
  const fetchValues = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get<{ values: ValueData[] }>('/api/values');
      setData(response.data.values || []);
    } catch (err: any) {
      const errorMessage =
        'Failed to fetch values: ' + (err.response?.data?.error || err.message || 'Unknown error');
  ...
  const columns: Column<ValueData>[] = useMemo(
    () => [
      {
        Header: '#',
        id: 'rowIndex',
        Cell: ({ row, rows }: { row: Row<ValueData>; rows: Row<ValueData>[] }) => {
      ...
      },
      {
        Header: 'Actions',
        accessor: 'value_id',
        Cell: ({ row }: { row: Row<ValueData> }) => (
          <ActionButtons value_id={row.original.value_id} onDeletionSuccess={fetchValues} />
        ),
      },
    ],
    [fetchValues]
  ...
  const tableInstance = useTable<ValueData>(
    {
      columns,
      data,
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, 'values.xlsx');
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Values Management</h2>
          <Link
            to="/add/value"
            ...
            <tbody {...getTableBodyProps()} className="divide-y divide-gray-200">
              {page.length > 0 ? (
                ...
                <tr>
                  ...
                  >
                    No values found matching your criteria.
                  </td>
                </tr>
              )}
            </tbody>
