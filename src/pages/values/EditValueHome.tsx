...
const EditValuesHome = () => {
  const navigate = useNavigate();
  const { values_home_id } = useParams<{ values_home_id: string }>();
  ...
  useEffect(() => {
    const fetchValuesHome = async () => {
      if (!values_home_id) {
        toast.error('Values Home ID is missing.');
        navigate('/value/home');
        return;
      }
      try {
        const response = await axiosInstance.get<ValuesHomeData>(`/api/values-home/${values_home_id}`);
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.put(`/api/values-home/${values_home_id}`, payload, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      toast.success(response.data.message || 'Values home entry updated successfully');
      setFormData((prev) => ({ ...prev, home_img: null }));
      setCurrentImage(response.data.slider?.home_img || null);
      setTimeout(() => navigate('/value/home'), 2000);
    } catch (error: any) {
