...
const ActionButtons: React.FC<ActionButtonsProps> = ({ values_home_id, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/values-home/${values_home_id}`);
      toast.success('Values home entry deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit/valuehome/${values_home_id}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this values home entry?</p>
...
const ValuesHome: React.FC = () => {
  ...
  const fetchValuesHomes = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('Values Home Entries', 20, 10);
    autoTable(doc, {
    ...
    doc.save('values_home_entries.pdf');
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'ValuesHomes');
    XLSX.writeFile(workbook, 'values_home_entries.xlsx');
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Values Home Management</h2>
          <Link to="/add/valuehome" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <input
            ...
            placeholder="Search values home entries..."
