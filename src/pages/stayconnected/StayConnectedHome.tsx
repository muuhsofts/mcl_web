...
const ActionButtons: React.FC<ActionButtonsProps> = ({ stay_connected_id, onDeletionSuccess }) => {
  ...
  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/stay-connected-home/${stay_connected_id}`);
      toast.success('Stay connected home entry deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete stay connected home entry.', { position: 'top-right' });
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit/stay-connectedhome/${stay_connected_id}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this stay connected home entry?</p>
...
export default function StayConnectedHome() {
  ...
  const fetchStayConnectedHomes = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Failed to fetch stay connected home entries.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('Stay Connected Home Entries', 20, 10);
    autoTable(doc, {
    ...
    doc.save('stay_connected_home_entries.pdf');
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'StayConnectedHomes');
    XLSX.writeFile(workbook, 'stay_connected_home_entries.xlsx');
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Stay Connected Home Management</h2>
          <Link to="/add/stayconnected/home" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <input
            ...
            placeholder="Search stay connected home entries..."
              ...
              {page.length > 0 ? (
                ...
                <tr>
                  <td colSpan={columns.length} className="text-center py-10 text-gray-500">
                    No stay connected home entries found matching your criteria.
                  </td>
                </tr>
              )}
            </tbody>
