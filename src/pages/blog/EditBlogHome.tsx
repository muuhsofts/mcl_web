...
const EditBlogHome = () => {
  ...
  useEffect(() => {
    const fetchBlogHome = async () => {
      if (!bloghome_id) {
        toast.error('Blog Home ID is missing.');
        navigate('/blog/home');
        return;
      }
      try {
        const response = await axiosInstance.get<BlogHomeData>(`/api/blog-home-sliders/${bloghome_id}`);
        setFormData({
        ...
        toast.error('Failed to fetch blog home entry');
