...
const AddBlogHome = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/blog-home-sliders', payload, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      toast.success(response.data.message || 'Blog home entry created successfully', {
        position: 'top-right',
      });
      setTimeout(() => navigate('/blog/home'), 2000);
    } catch (error: any) {
      const errorMessage = error.response?.data?.error || 'Failed to create blog home entry';
