...
const EditBlog = () => {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      setFetching(true);
      try {
        const response = await axiosInstance.get(`/api/blogs/${blog_id}`);
        const { heading, description } = response.data.blog;
        setFormData({ heading, description: description || '' });
      } catch (error: any) {
        const errorMessage = error.response?.data?.error || 'Failed to fetch blog entry';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.put(`/api/blogs/${blog_id}`, formData);
      toast.success(response.data.message || 'Blog entry updated successfully', {
        position: 'top-right',
      });
      setTimeout(() => navigate('/blogs'), 2000);
    } catch (error: any) {
      const errorMessage = error.response?.data?.error || 'Failed to update blog entry';
