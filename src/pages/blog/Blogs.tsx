...
const ActionButtons: React.FC<ActionButtonsProps> = ({ blog_id, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/blogs/${blog_id}`);
      toast.success('Blog entry deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete blog entry.', { position: 'top-right' });
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit/blog/${blog_id}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this blog entry?</p>
...
export default function Blogs() {
  ...
  const fetchBlogs = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = 'Failed to fetch blog entries: ' + (err.response?.data?.error || err.message || 'Unknown error');
      setError(errorMessage);
      toast.error('Failed to fetch blog entries.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, 'blog_entries.xlsx');
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Blog Management</h2>
          <Link to="/add/blogs" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
              ...
              {page.length > 0 ? (
                ...
                <tr>
                  <td colSpan={columns.length} className="text-center py-10 text-gray-500">
                    No blog entries found matching your criteria.
                  </td>
                </tr>
              )}
            </tbody>
