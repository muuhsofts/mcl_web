...
const ActionButtons: React.FC<ActionButtonsProps> = ({ contactId, onDeletionSuccess }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/api/contact-us/${contactId}`);
      toast.success('Contact record deleted successfully!', { position: 'top-right' });
      onDeletionSuccess();
    } catch (err) {
      toast.error('Failed to delete contact record.', { position: 'top-right' });
  ...
  return (
    <div className="relative flex items-center gap-2">
      <Link to={`/edit/contact-us/${contactId}`} className="p-1 text-blue-500 hover:text-blue-600" aria-label="Edit">
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-600 mb-6">Are you sure you want to delete this contact record?</p>
...
const UrlLinkCell: React.FC<{ value: string | null }> = ({ value }) => {
  if (!value) return <span className="text-gray-500 text-sm">No link</span>;
  return (
...
export default function ContactUs() {
  ...
  const fetchContacts = useCallback(async () => {
    ...
    try {
      ...
      setData(response.data.contacts || response.data);
    } catch (err: any) {
      const errorMessage = 'Failed to fetch contact records: ' + (err.response?.data?.error || err.message || 'Unknown error');
  ...
  const columns: readonly Column<ContactData>[] = useMemo(
    () => [
      {
        Header: '#',
        id: 'rowIndex',
        Cell: ({ row, flatRows }: CellProps<ContactData>) => {
      ...
      { Header: 'Category', accessor: 'category' },
      {
        ...
        Cell: ({ value }: CellProps<ContactData, string | null>) => <DescriptionCell value={value} />,
      },
      {
        Header: 'Image',
        accessor: 'img_file',
        Cell: ({ value }: CellProps<ContactData, string | null>) => {
      ...
      },
      {
        Header: 'URL Link',
        accessor: 'url_link',
        Cell: ({ value }: CellProps<ContactData, string | null>) => <UrlLinkCell value={value} />,
      },
      {
        Header: 'Created At',
        accessor: 'created_at',
        Cell: ({ value }: CellProps<ContactData, string>) => new Date(value).toLocaleDateString(),
      },
      {
        Header: 'Actions',
        id: 'actions',
        Cell: ({ row }: CellProps<ContactData>) => (
          <ActionButtons contactId={row.original.contactus_id} onDeletionSuccess={fetchContacts} />
        ),
      },
    ],
    [fetchContacts]
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('Contact Records', 20, 10);
    autoTable(doc, {
      head: [['#', 'Category', 'Description', 'URL Link', 'Created At']],
    ...
    <div className="container mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Contact Us Management</h2>
          <Link to="/add/contact-us" className="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition shadow-md">
          ...
          <table {...getTableProps()} className="min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
            ...
            <tbody {...getTableBodyProps()} className="divide-y divide-gray-200">
              {page.length > 0 ? (
                ...
                <tr><td colSpan={columns.length} className="text-center py-10 text-gray-500">No contact records found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {data.length > 0 && page.length > 0 && (
          <div className="flex flex-col sm:flex-row justify-between items-center mt-6 gap-4">
            <div className="flex gap-2">
              <button onClick={() => previousPage()} disabled={!canPreviousPage} className="px-4 py-2 bg-blue-500 text-white rounded-lg disabled:opacity-50 hover:bg-blue-600 transition">Previous</button>
              <button onClick={() => nextPage()} disabled={!canNextPage} className="px-4 py-2 bg-blue-500 text-white rounded-lg disabled:opacity-50 hover:bg-blue-600 transition">Next</button>
            </div>
            <div className="text-sm text-gray-700">
